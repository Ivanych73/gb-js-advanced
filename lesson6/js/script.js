...
Vue.component('goods-list', {
  ...
  template: `
    <div class="goods-list">
      <goods-item v-for="good in goods" :good="good"  :addtocartitem="addToCart"></goods-item>
      <server-error :visible="servererrorvisible"></server-error>
      <search-error :goods="goods"></search-error>
    </div>
    `,
  methods: {
    addToCart(good) {
...
Vue.component('server-error', {
  ...
  template:`
    <div v-if="visible">
      <p>Не удается получить спиок товаров с сервера! Возможно сервер сейчас недоступен, либо чересчур перегружен, либо возникла проблемы с интернет-подключением!</p>
    </div>
  `
})
...
Vue.component('cart-div', {
  ...
  template: `
    <div class="cart" v-if="visible">
      <h2>Корзина</h2>
      <ul class="cart__list">
        <cart-item v-for="good in goods" :good="good" :addtocartitem="addToCart" :removefromcartitem="removeFromCart"></cart-item>
      </ul>
...
const app = new Vue({
  ...
  methods: {
    ...
    addToCart(good) {
      ...
      if (!currentCartItem) {
        this.cart.goods.push({title: good.title, price: good.price, id: good.id, quantity: 1});
    ...
    removeFromCart(good) {
      ...
      if (currentCartItem.quantity > 1) {
        ...
        this.cart.goods.splice(this.cart.goods.indexOf(currentCartItem));
      }
